...
''
  ...
  // Start GVFS and Thunar daemons
  spawn-at-startup "/usr/lib/gvfs/gvfsd"
  spawn-at-startup "/usr/lib/gvfs/gvfsd-fuse /run/user/$(id -u)/gvfs"
  spawn-at-startup "thunar --daemon"
  spawn-at-startup "thunar-volman --daemon"
  ...
  window-rule {
      match app-id="installer$"
      match app-id="dropterm$"
  ...
  binds {
     ...
     Mod+I hotkey-overlay-title="Power Menu" {
     ...
     Mod+Shift+N allow-when-locked=true {
      ...
      // custom bind to launch applications
      Mod+Return { spawn "kitty"; }
      Mod+Shift+E { spawn "thunar"; }
      // Mod+I { spawn "wlogout"; }
      Mod+B { spawn "zen-browser"; }

      Mod+E { spawn-sh "$HOME/.config/scripts/yazi.sh"; }
      Mod+P { spawn-sh "$HOME/.config/scripts/rofi/list-installer.sh"; }
  }
''
